"""
FastAPI Server for Content Compliance RL Environment.

Provides REST API endpoints for OpenEnv compliance:
- POST /reset - Reset environment and start new episode
- POST /step - Take action in environment
- GET /health - Health check endpoint

All endpoints return valid JSON and respond with HTTP 200 on success.
...
async def health_check():
    """Health check endpoint. Returns HTTP 200 with server status."""
...
async def reset(request: Request):
    ...
    Accepts empty request or optional JSON body with:
    - max_steps: int (default: 5)
    - difficulty: str (default: "mixed")

    Returns HTTP 200 with Observation JSON.
    """
